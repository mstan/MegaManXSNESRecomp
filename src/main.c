 ...
 * -DMMX_RECOMP_UI=ON; its recomp_ui.cmake defines RECOMP_LAUNCHER. MMX drives
...
static void SdlRenderer_BeginDraw(int width, int height, uint8 **pixels, int *pitch) {
  ...
  snesrecomp_sdl_get_texture_size(
      g_texture, &texture_width, &texture_height);
...
static void SdlRenderer_EndDraw(void) {
  ...
  snesrecomp_sdl_render_texture(
      g_renderer, g_texture, &g_sdl_renderer_rect,
      &g_sdl_present_rect);
...
int main(int argc, char** argv) {
  ...
  int mods_ready = 0;
  {
     ...
     * MSU-1 are Mods features, not generic settings controls. */
    {
      ...
      if (want_launcher) {
        ...
        gi.msu1_supported = 0;          /* MSU-1 is configured by the Mods package. */
...
static const char kDefaultConfigIniContent[] =
  ...
  "# Works with SDL and OpenGL, and is exposed in the launcher Settings page.\n"
  "LinearFiltering = 0\n"
  "\n"
